...
- packed `v0.2.1` tarball installs into a fresh temp project, runs `replaypack brief` and `replaypack trial`, and writes agent-brief/receipt/feedback/agent-report files
...
| Package cleanliness | 95 | 85 | pass | npm package allowlist is explicit; packed tarball installs, runs `replaypack trial`, and writes feedback plus agent-report artifacts |
